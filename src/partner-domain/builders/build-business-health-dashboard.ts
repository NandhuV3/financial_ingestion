...
export function buildBusinessHealthDashboard(
  ...
  if (artifacts.healthDashboardEnriched) {
    return {
      status,
      explanation: artifacts.healthDashboardEnriched.explanation,
      strengtheningAreas: artifacts.healthDashboardEnriched.strengthening_areas.slice(0, 3),
      watchAreas: artifacts.healthDashboardEnriched.watch_areas.slice(0, 3),
      timeline,
    };
  }
...
function buildWatchAreas(artifacts: PartnerSourceArtifacts): string[] {
  ...
  const fromEvolution = artifacts.topicEvolution?.topics
    ...
    .map((topic) => topic.topic_name) ?? [];
  const fromForensics = artifacts.insight?.risks.map(riskLabel) ?? [];
