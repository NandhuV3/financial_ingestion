...
import { getCompetitiveAdvantages } from "../../company-profile/company-profile-accessors.js";

export function buildTrustProfile(artifacts: PartnerSourceArtifacts): TrustProfile {
  const customers = artifacts.companyProfile.customers.slice(0, 2).join(" and ") || "customers";
  const advantages = getCompetitiveAdvantages(artifacts.companyProfile).slice(0, 2).join(" and ") || "durable business strengths";
