...
import {
  EVIDENCE_CATALOG_SECTION_ORDER,
...
import type {
  ...
  EvidenceCatalogDependencies,
} from "./types.js";
import { stableHash } from "../investor-intelligence-builder/hashes.js";

export function validateEvidenceCatalogBuilderInput(
  input: EvidenceCatalogBuilderInput,
): void {
...
export function validateEvidenceCatalogBuildTarget(input: {
  builderInput: EvidenceCatalogBuilderInput;
  companyId: string;
  periodId: string;
}): void {
  if (input.builderInput.filing_artifact.filing_period !== input.periodId) {
    throw new BuilderValidationError(
      "Evidence Catalog filing period must match the build target.",
    );
  }
...
export function resolveEvidenceCatalogDependencies(input: {
  ...
  if (!filing || filing.identity.artifact_type !== "filing") {
    throw new BuilderValidationError(
      "Evidence Catalog requires the Filing Artifact dependency.",
    );
  }
  ...
  if (
    ...
    throw new BuilderValidationError(
      "Evidence Catalog Filing Artifact dependency does not reconcile with the build target and input.",
    );
  }
...
export function validateEvidenceCatalogContent(input: {
  builderInput: EvidenceCatalogBuilderInput;
  content: EvidenceCatalogArtifactContent;
  companyId: string;
  periodId: string;
}): void {
  if (input.content.artifact_type !== "evidence_catalog") {
    throw new BuilderValidationError(
      "content.artifact_type must be evidence_catalog.",
    );
  }

  if (
    input.content.company_id !== input.companyId
    || input.content.period_id !== input.periodId
    ...
    throw new BuilderValidationError(
      "Evidence Catalog identity does not reconcile with the Filing Artifact.",
    );
  }
  ...
  validateEntries(input.content.entries);

  const expectedEntries = buildEvidenceCatalogEntries(
    input.builderInput.filing_artifact,
  );

  if (stableHash(input.content.entries) !== stableHash(expectedEntries)) {
    throw new BuilderValidationError(
      "Evidence Catalog entries do not reconcile with deterministic generation.",
    );
  }
}

function validateEntries(entries: EvidenceCatalogEntry[]): void {
  ...
  const nextParagraphIndex = new Map<EvidenceSectionName, number>(
    EVIDENCE_CATALOG_SECTION_ORDER.map((section) => [section, 1]),
  );
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (
      !EVIDENCE_CATALOG_SECTION_ORDER.includes(entry.section_name as never)
    ) {
      throw new BuilderValidationError(
    ...
    const sectionIndex = EVIDENCE_CATALOG_SECTION_ORDER.indexOf(sectionName);
    ...
    if (
      ...
      throw new BuilderValidationError(
        "Evidence Catalog ordering is invalid.",
      );
    }
