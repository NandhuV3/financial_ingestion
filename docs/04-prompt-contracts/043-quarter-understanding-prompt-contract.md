...
This contract governs Quarter Understanding prompt execution.
...
Quarter Understanding Builder invokes the governed prompt using the active
builder flow defined in 054-quarter-understanding-builder-spec.md.
...
Prompt output must produce content only:

```typescript
type QuarterUnderstandingPromptOutput = {
  understandings:
    UnderstandingEntry[];
...
Artifact Framework metadata, Artifact Framework lineage, identity, versioning,
persistence, current pointer, archive/history, and framework hashes must not be
generated by the prompt.

The prompt owns interpretation reasoning and emits only:
...
The Quarter Understanding Builder owns:
...
When Concept Registry enrichment is unavailable:

```text
concept_id must be absent.
```

Concept Registry availability is assembled and recorded by the builder.
...
Prompt Registry supplies the governed prompt metadata used for execution.

The Quarter Understanding Builder records that metadata in content-level
replayability metadata:
...
Prompt output does not emit prompt lineage.

Quarter Understanding replayability metadata is not Artifact Framework
lineage. Artifact Framework lineage remains separately owned and assembled by
Artifact Framework.
