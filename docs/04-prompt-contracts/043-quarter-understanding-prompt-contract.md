...
This contract records the Quarter Understanding prompt boundary for future LLM
execution.

Sprint 10 active runtime decision:

```text
Quarter Understanding is deterministic.
```

The Sprint 10 builder must not invoke LLMs, resolve runtime prompts, or embed
...
# Future Prompt Output Contract

If a future architecture enables LLM execution for Quarter Understanding, prompt
output must produce content only:
